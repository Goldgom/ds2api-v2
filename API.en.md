...
- Within one assistant message, `tool_calls[].index` increases globally: `continue` rounds keep advancing the index, and the `id` bound to an index is never reused across rounds. OpenAI clients merge deltas by `index`, so distinct calls never share `index: 0`.
- Upstream resends the whole message as a snapshot at the start of every `continue` round; that replay is deduplicated so the same tool block is not emitted as a second call. A replay delivered token by token, or fragment by fragment (one whole fragment per part, where the part that restarts the message may be plain prose), is aligned against the already accumulated text and dropped as well, so repeated calls do not accumulate per round. If the model intentionally calls the same tool twice with the same arguments in one round, both calls are still emitted. Dedupe is one-sided: only replayed text is dropped, accumulated text is never cut on a guess. If the upstream *re-renders* the whole message into a version that differs from what was already streamed (no captured sample of that exists), that version is appended instead: the client may see the body again plus one call with valid arguments and a unique id/index. That direction is deliberate - a call with correct arguments is never traded for one with mismatched ones.
