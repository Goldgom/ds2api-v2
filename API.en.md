...
- Upstream resends the whole message as a snapshot at the start of every `continue` round; that replay is deduplicated so the same tool block is not emitted as a second call. If the model intentionally calls the same tool twice with the same arguments in one round, both calls are still emitted.
