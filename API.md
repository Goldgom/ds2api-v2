...
- 同一条 assistant 消息内 `tool_calls[].index` 全局递增：跨 `continue` 轮次会接着已有 `index` 继续分配，`id` 与 `index` 绑定且不在轮次之间复用。OpenAI 客户端按 `index` 合并增量，因此不同调用不会共用 `index: 0`。
- `continue` 轮次开头上游会重发整条消息快照，这类重放会被去重：同一个工具块不会被发射成第二个调用。上游按 token 逐段重放、或**按片段重放整条消息**（一块=一个片段，重启消息的那块可能只是正文）时，同样会按已累积文本对齐后丢弃，因此重复调用不会随轮次增加。模型在同一轮内有意用相同参数重复调用同一工具时，两次都会照常输出。去重是单向的：只丢弃重放内容，不会为了“猜到是重放”而裁剪已经下发过的正文；如果上游把整条消息**重新渲染**成与已下发内容不同的版本（当前无实测样本），该版本会被追加，客户端可能再收到一次正文与一个**参数合法、id/index 唯一**的调用——这是刻意保留的安全方向，绝不会产出参数错位的调用。
