...
`continue` 轮次开头重发的完整消息快照属于**同一轮内容的副本**，不是新内容。实测与样本共同确认的两点：
...
反向的坑同样要记住：一条消息里连续几个**相似的工具调用**（同一工具、相同 `displayName`/`intent`、重复的参数文本）本身就会与消息开头或彼此共享很长的相同字节。任何只看文本重合的重放判定，都会把后面的调用误当成“重写快照”，于是裁掉已有累积文本——客户端看到的就是某个调用拿到了另一个调用的参数、部分调用消失、原始标记泄漏进正文。因此现在的去重规则是**单向**的：只丢弃入块重放的部分，永不裁剪已累积文本；整消息快照由解析层打标 + 行泵单独成块来结构化识别；逐 token 重放只允许“片段重启整条消息 + 含工具标记”作为开对齐证据。已验证的回归：畸形块（缺闭合标签）后紧跟合法工具块时，合法块的尾部标记不会被当成正文下发（`TestToolCallStabilityRisks/good-call-after-malformed-block`）。
